...
export const createGame = async (req, res) => {
    try {
        const {titulo, descripcion, precio, precio_descuento, fecha_lanzamiento, portada_url, genero,stock, plataforma, edad_minima} = req.body;

        if(!titulo || !precio){
            res.status(400).json({ message: 'Faltan datos obligatorios' })
            return;
        }
        const nuevoJuego = await createGameModel({titulo, descripcion, precio, precio_descuento, fecha_lanzamiento, portada_url, genero,stock, plataforma, edad_minima})
...
export const updateGame = async (req, res) => {
    try {
        ...
        const {titulo, descripcion, precio, precio_descuento, fecha_lanzamiento, portada_url, genero,stock, plataforma, edad_minima, activo} = req.body;

        const juegoActualizado = await updateGameModel({titulo, descripcion, precio, precio_descuento, fecha_lanzamiento, portada_url, genero,stock, plataforma, edad_minima,activo, id});
